package JavaProgramingFundamentals.ObjectsAndClasses.Lab;
...
public class P06Students2 {
    ...
    public static void main(String[] args) {
        ...
        while (!input.equals("end")) {
            ...
            if (isStudentExist(studentList, currentStudent) >= 0) {
                int index = isStudentExist(studentList, currentStudent);
                studentList.set(index,currentStudent);
            } else {
    ...
    private static int isStudentExist(List<Student> studentList, Student currentStudent) {
        for (int i = 0; i < studentList.size(); i++) {
            if (studentList.get(i).getFirstName().equals(currentStudent.getFirstName())
                    && studentList.get(i).getLastName().equals(currentStudent.getLastName())) {
                return i;
            }
        }
        return -1;
